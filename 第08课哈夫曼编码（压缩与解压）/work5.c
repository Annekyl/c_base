...
typedef struct Node {
    char name[NAME_LENGTH];  // 存放汉字字符
    int weight;  // 存放权重
    int left;  // 左孩子
    int right;  // 右孩子
    int parent;  // 父节点
    char code[CODE_LENGTH];  // 哈夫曼编码
} HTNode, *HuffmanTree;

int n = 0;  // 节点个数
char leaf_name[6000][NAME_LENGTH];   // 节点字符
int leaf_weight[6000]; // 节点权重
char huf[6000][2][50];  // 存放字符及其对应的哈夫曼编码
int hu = 0; // 索引，用于查找字符
...
void decompress(HuffmanTree HT) {
    // 打开文件
    FILE *file4 = fopen("词频压缩.txt", "rb");
    if (file4 == NULL) {
        printf("压缩文件打开失败");
        exit(1);
    }
    FILE *file5 = fopen("词频解压.txt", "w");
    if (file5 == NULL) {
        printf("解压文件打开失败");
        exit(1);
    }
